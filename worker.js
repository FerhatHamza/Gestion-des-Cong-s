// worker.js – sans npm, sans import externe
// Utilise un routeur maison basé sur URLPattern
...
function getDB(env) { return env.DB; }
...
async function signJWT(payload) {
  ...
  const key = await crypto.subtle.importKey(
    'raw', encoder.encode('votre-clef-secrete-tres-longue'),
    { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
  const header = { alg: 'HS256', typ: 'JWT' };
  const now = Math.floor(Date.now() / 1000);
  const jwtPayload = { ...payload, exp: now + 86400 }; // 24h
  ...
  try {
    ...
    if (path === '/api/auth/login' && method === 'POST') {
      ...
      if (emp) {
        ...
        return json({ token, role: 'employee', firstLogin: emp.first_name === '' || emp.first_name === null });
      }
    ...
    if (path === '/api/employee/profile' && method === 'GET') {
      ...
      const emp = await getDB(env).prepare(`
        ...
        FROM employees e
        JOIN grades g ON e.grade_id=g.id JOIN degrees d ON e.degree_id=d.id
        JOIN services s ON e.service_id=s.id JOIN centers c ON e.center_id=c.id
    ...
    if (path === '/api/leaves/request' && method === 'POST') {
      ...
      let start = new Date(start_date), end, duration;
      ...
      if (overlap.count > 0) return json({ error: 'Chevauchement' }, 400);
      // Solde si annuel
    ...
    if (validateMatch && method === 'PUT') {
      ...
      await getDB(env).prepare('UPDATE leave_requests SET status=?, validated_by=?, validated_at=CURRENT_TIMESTAMP WHERE id=?').bind(status, user.id, requestId).run();
      await getDB(env).prepare('INSERT INTO audit_logs (user_id, action, details) VALUES (?,?,?)').bind(user.id, 'validate_request', JSON.stringify({ requestId, status })).run();
      return json({ success: true });
    }
